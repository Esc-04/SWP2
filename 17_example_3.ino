...
#define _DUTY_MIN 552  // servo full clock-wise position (0 degree)
...
void loop()
{
  ...
  dist_ema = dist_filtered*EMA_ALPHA+(1-EMA_ALPHA)*dist_ema;// Put EMA filter code here
      
  //duty = map(dist_ema, _DIST_MIN, _DIST_MAX, _DUTY_MIN, _DUTY_MAX);
  duty =(int)(_DUTY_MIN+(_DUTY_MAX-_DUTY_MIN)*(dist_ema-_DIST_MIN)/(_DIST_MAX-_DIST_MIN));// Put map() equivalent code here
